...
CF_IP_TMP_DIR="/tmp"
[ -d "/data/data/com.termux/files/home" ] && CF_IP_TMP_DIR="/data/data/com.termux/files/home"
CF_IP_DIR="${CF_IP_TMP_DIR}/cloudflare_ip"
CF_RESULT_FILE="${CF_IP_TMP_DIR}/cloudflare_result.txt"

fetch_cf_ips() {
	DL_CF_IP_TO="${CF_IP_TMP_DIR}/cf_ips.zip"
	mkdir -p "$CF_IP_DIR"
	rm -rf "$CF_IP_DIR"/*
...
result_append() {
	[ "$ENABLE_LOG" = "0" -o "$ENABLE_LOG" = "false" ] && return 0
	echo "$1" >> "$CF_RESULT_FILE"
...
if [ "$#" = "0" -o -z "$1" ]; then
	...
	cat "$CF_RESULT_FILE" 2>/dev/null | sort -rn
